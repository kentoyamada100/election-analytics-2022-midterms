...
```{r nation-level prediction data, include = FALSE}
# Data on GDP/quarter
...
```{r finalize national_df, include = FALSE}
national_df <- full_join(gdp_df_final, rdi_df_final, by = "election_year") %>%
...
```{r national_df_2020, include = FALSE}
# Discard 2020
national_df_without_2020 <- national_df[national_df$election_year != 2020,]
```

```{r nation-level model, include = FALSE}
national_model <- 
  lm(pp_vote_share ~ gdp_q7_q6 + midterm + pp_generic_ballot, 
        data = national_df_without_2020)
```
...
```{r predictions data, include = FALSE}
# Get data for 2022
latest_data <- 
...
```{r}
predict(national_model, data = latest_data, interval = "prediction")
```
...
convert_nat_dist <- lm(pct_D_rel ~ rating_avg + d_ad_rate + d_inc, data = df_2018)
```
