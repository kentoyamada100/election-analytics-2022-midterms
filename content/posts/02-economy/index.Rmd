...
By comparing the predicted values based on Models (A) ~ (D) with the actual vote share, we can see that Models (C) and (D) tend to better predict the vote share (Figure 2). Indeed, the adjusted $R^2$ for Models (C) and (D) are higher than the adjusted $R^2$ for Models (A) and (B). Also, the root-mean-squared errors are lower for Models (C) and (D) (See Figure 3 for details).
