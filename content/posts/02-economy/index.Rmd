...
`r round(cor.coef, 2) ` and (p-value: r format(cor.test.results$p.value, 2, scientific = TRUE, digits = 2)`)


`r round(cor.coef.2, 2) `  (p-value: `r format(cor.test.results.2$p.value, 2, scientific = TRUE, digits = 2)`)


#### What's wrong with these models?: Outlier

```{r outlier, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Figure 1: GDP Growth vs RDI Growth: Q7-Q8"}
...
```{r plot residuals, echo = FALSE, fig.cap = "Actual Values vs Predicted Values"}
...
```{r combine table with plots, fig.cap = "Model Testing & Predictions", echo = FALSE}
