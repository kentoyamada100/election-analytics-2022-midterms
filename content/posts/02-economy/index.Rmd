...
lines(df$election_year[-c(1:6, length(df$election_year))], 
       predict(modelA, df_without_2020)[-(1:6)], col = "red", lty = "dotted") 

# Predicted values based on Model B
lines(df$election_year[-length(df$election_year)], #De-select 2020
...
# Calculate prediction errors, take mean, then take square root
...
os_modelA_pred <- predict(os_modelA, 
                          df_without_2020[df_without_2020$election_year == 2018,])
...
os_modelB <- lm(pp_vote_share ~ gdp_q7_q6  + midterm, 
                     data = df_without_2020[df_without_2020$election_year != 2018,])
os_modelB_pred <- predict(os_modelB, 
                          df_without_2020[df_without_2020$election_year == 2018,])
...
os_modelC <- lm(pp_vote_share ~ gdp_q7_q6 + midterm + unrate_q7, 
                     data = df_without_2020[df_without_2020$election_year != 2018,])
os_modelC_pred <- predict(os_modelC, 
                          df_without_2020[df_without_2020$election_year == 2018,])
...
os_modelD <- lm(pp_vote_share ~ gdp_q7_q1 + midterm + unrate_q7, 
                     data = df_without_2020[df_without_2020$election_year != 2018,])
os_modelD_pred <- predict(os_modelD, 
                          df_without_2020[df_without_2020$election_year == 2018,])
...
model_eval[,8] <- as.numeric(model_eval[,8])
```
...
predictions <- rbind(modelA_pred, modelB_pred, modelC_pred, modelD_pred)
```
